...
B_VERSION, B_PREV_HASH, B_DIFF_TARGET, B_NONCE, B_FILES, B_TRANSACTIONS, = \
    ...
    [File('fcontent', 'fname')], [Transaction('1', '2'), Transaction('3', '4')]
...
@pytest.fixture()
def mblock():
    return Block(B_VERSION, B_PREV_HASH, B_DIFF_TARGET, B_NONCE, transactions=B_TRANSACTIONS, files=B_FILES)
...
@pytest.fixture(
    params=[{'total_size': i, 'n_files': j, 'exp_out': k} for i, j, k in
            [(Block.BLOCK_FILE_SIZE - 8, 1, False), (Block.BLOCK_FILE_SIZE - 12, 4, True),
             (Block.BLOCK_FILE_SIZE + 8, 8, False), (Block.BLOCK_FILE_SIZE - 24, 8, True)]]
)
def tmp_files(tmp_path, request):
    total_size, n_files, expected_output = request.param['total_size'], request.param['n_files'], request.param[
        'exp_out']
    ...
    for i in range(n_files):
        ...
        with open(file_path, 'wb') as fp:
            random_content = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(f_size))
            fp.write(random_content.encode())
    return files, expected_output


def test_block_params(mblock: Block):
    assert mblock.version == B_VERSION and mblock.prev_hash == B_PREV_HASH \
           and mblock.difficulty_target == B_DIFF_TARGET and mblock.nonce == B_NONCE \
           and mblock.transactions == B_TRANSACTIONS and mblock.files == B_FILES
...
def test_block_to_dict(mblock: Block):
    assert mblock.version == B_VERSION and mblock.prev_hash == B_PREV_HASH \
           and mblock.difficulty_target == B_DIFF_TARGET and mblock.nonce == B_NONCE \
           and mblock.files == B_FILES and mblock.transactions == B_TRANSACTIONS
...
def test_is_valid1(tmp_files):
    ...
    block = Block(version, prev_hash, difficulty_target, nonce, transactions=B_TRANSACTIONS, files=[])

    for fp in files:
        block.upload_file(fp)

    expected_out = is_files_size_valid and (version is not None) and (prev_hash is not None) and (
            difficulty_target is not None) and (nonce is not None)
