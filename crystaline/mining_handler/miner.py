...
class Miner:
    def __init__(self, blockchain:Blockchain, file_pool:list, transaction_pool:dict) -> None:
    ...
    def mine_block(self):
        ...
        transactions_to_be_mined = self.get_items_to_be_mined(self.transaction_pool.values(), BLOCK_TRANSACTION_SIZE)
        ...
        for i in range(NONCE_RANGE[0], NONCE_RANGE[1]):
            ...
            if block.is_valid():
                ...
                self.transaction_pool = {k: v for k, v in self.transaction_pool.items() if v not in transactions_to_be_mined}
                return block
        return None
